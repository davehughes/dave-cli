...
class CLI(object):
    ...
    def build_arg_parser(self):
        parser = argparse.ArgumentParser(
            "dave", description="cli and desktop control helpers"
        )
        parser.add_argument(
            "--log-file",
            type=str,
    ...
    def add_odc_commands(self, subparsers):
        def cmd_move_space(opts):
            ...
            try:
                self.grid_controller.move(
                    **grid.MOVEMENT_DIRECTION_OFFSETS[opts.direction]
                )
            except Exception as e:
                logging.error(e)

        move_space_cmd = subparsers.add_parser(
            "odc-move-space", help="Move between spaces"
        )
        ...
        move_window_cmd = subparsers.add_parser(
            "odc-move-window", help="Move focused window"
        )
        ...
        invert_space_cmd = subparsers.add_parser(
            "odc-invert-space", help="Flip the tree in the current space"
        )
        ...
        set_space_layout_cmd = subparsers.add_parser(
            "odc-set-space-layout", help="Set the layout mode for the current space"
        )
    ...
    def add_mfa_commands(self, subparsers):
        ...
        list_cmd = subparsers.add_parser(
            "mfa-list", help="List all stored MFA profiles"
        )
    ...
    def add_pg_commands(self, subparsers):
        # pg-list
        def cmd_list_databases(_opts):
            pgpass.list_databases()

        list_cmd = subparsers.add_parser(
            "pg-list", help="Print a list of all stored pgpass profiles"
        )
        ...
        connect_cmd = subparsers.add_parser(
            "pg-print-psql-connection-command",
            help="Print the psql command for connecting to the specified profile",
        )
        ...
        connect_cmd = subparsers.add_parser(
            "pg-print-dbext",
            help="Dump a let statement for defining a default profile in vim's dbext",
        )
        ...
        connect_cmd = subparsers.add_parser(
            "pg-print-dadbod",
            help="Print a statement to connect to the named profile via dadbod.vim",
        )
        connect_cmd.add_argument("alias", nargs="?")
