...
ConnectionLine = collections.namedtuple(
    "ConnectionLine", ["host", "port", "database", "user", "password"]
)
...
def list_databases(filepath=None):
    ...
    for alias in aliases:
        ...
        print(
            f"{alias}: {connection_line.user}:***@{connection_line.host}:{connection_line.port}/{connection_line.database}"
        )


def dump_dbext(alias=None):
    config_template = (
        "let g:dbext_default_profile_{profile_name} = '{connection_string}'"
    )
